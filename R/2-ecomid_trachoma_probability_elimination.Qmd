...
#-------------------------------
d <- read_rds(here("data","ecuador_trachoma_elimination_data_2026-09-14.rds"))
...
d <- d %>%
  ...
  mutate(community_name = ifelse(community_name == "Borbon","Borbón",community_name)) %>%
  mutate(community_type = factor(community_type, levels = c("Rural Villages","Esmeraldas and Borbon"),labels = c("Rural Villages","Borbón")))
...
rural_comm_sorted <- comm_sorted[comm_sorted!="Borbón"]
dprev_comm_plot <- dprev_comm %>%
  mutate(community_name_plot = factor(community_name, levels = c("Borbón",rural_comm_sorted)),
         community_type = ifelse(community_name == "Borbón","Borbón","Rural villages"),
         community_type = factor(community_type, levels = c("Rural villages","Borbón"))
         )
...
plot_seroprev_community <- ggplot(data = dprev_comm_plot, aes(x = community_name_plot, y = seroprev, color = community_type)) +
  ...
  labs(x = "Community", y = "Pgp3 IgG Seroprevalence (%)", tag = "C") +
...
#-------------------------------
d_urban <- d %>% filter(community_type == "Borbón")
...
d_loess_fits <- data.frame(community_type = c(rep("Borbón",length(pred_ages)),rep("Rural Villages",length(pred_ages))),
...
plot_hist_age <- ggplot(data = d, aes(x = agey, fill = community_type)) +
  ...
  labs(x="Age, years",y = "N samples", tag="A") +
  theme_classic() +
  theme(
...
age_seroprev_comp <- plot_hist_age + plot_smooth_prev + plot_seroprev_community   + plot_layout(ncol = 1, heights=c(1,1,1))

# ggsave(filename = here("output","ecuador_pgp3_age_serprev.png"),age_seroprev_comp, device = "png", width = 90, height = 240, units = "mm",dpi=300)

# EID format, 300dpi at 5 inches wide TIF, or PDF. Use PDF to keep scaling.
ggsave(
  filename = here("output","ecuador_pgp3_age_serprev.pdf"),
  plot=age_seroprev_comp,
  device = "pdf",
  width = 90,
  height = 240,
  units = "mm"
...
# by urbanicity
#-------------------------------
d2_urban <- d2 %>% filter(community_type == "Borbón")
...
den_urban <- data.frame(eu_name = "Borbón", scr=dist_urban$x, eu_den = dist_urban$y)
...
plot_scr_dens <- ggplot(data=prob_eu, aes(x=scr, y=pool_den, fill=eu_name, linetype = trachoma_cat)) +
  ...
  labs(x="SCR per 100 person-years",y="Probability density") +
       #   title="Probability distributions of trachoma categories",
       # subtitle="Heavy black lines are SCR likelihoods for Ecuador"
  guides(fill = guide_legend(title="",override.aes = list(alpha = 1)), linetype = "none") +
...
sims_urban <- data.frame(scr = scr_sim_urban, eu_name = "Borbón")
...
plot_eu_den <- ggplot(data=ecuador_scr_sim) +
  ...
  # annotate("text",x=8.2,y=2.7,label="Rural Villages",color=cbpal[4]) +
  # annotate("text",x=1,y=2.1,label="Borbón",color=cbpal[2]) + 
  ...
  labs(y="",x="SCR per 100 person-years") +
  coord_cartesian(xlim=c(0,10),ylim=c(1,2.45)) +
  theme_classic() +
  theme(
    legend.position = "none",
    panel.grid.minor.x= element_blank(),
...
# ggsave(filename = here("output","ecomid_scr_densitities.png"),plot = combined_density_figure, device="png",width=220, height=90, units="mm",dpi=300)

# EID requirements, 300dpi at 5 inches wide, TIF format, or PDF
ggsave(
  filename = here("output", "ecomid_scr_densities.pdf"),
  plot = combined_density_figure,
  device = "pdf",
  width = 220,
  height = 90,
  units = "mm"
